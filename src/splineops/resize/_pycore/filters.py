...
def spline_poles(deg: int) -> np.ndarray:
    ...
    if deg == 2:
        return np.array([np.sqrt(8.0) - 3.0])
    elif deg == 3:
        return np.array([np.sqrt(3.0) - 2.0])
    elif deg == 4:
        return np.array(
            [
...
def sampling_fir(deg: int) -> np.ndarray:
    if deg <= 1:
        return np.array([], dtype=float)
    if deg == 2:
        return np.array([3.0 / 4.0, 1.0 / 8.0])
    elif deg == 3:
        return np.array([2.0 / 3.0, 1.0 / 6.0])
    elif deg == 4:
        return np.array([115.0 / 192.0, 19.0 / 96.0, 1.0 / 384.0])
    elif deg == 5:
        return np.array([11.0 / 20.0, 13.0 / 60.0, 1.0 / 120.0])
    elif deg == 6:
        return np.array(
            [5887.0 / 11520.0, 10543.0 / 46080.0, 361.0 / 23040.0, 1.0 / 46080.0]
        )
    elif deg == 7:
        return np.array([151.0 / 315.0, 397.0 / 1680.0, 1.0 / 42.0, 1.0 / 5040.0])
    else:
...
def symmetric_fir(h: Sequence[float], c: np.ndarray, s: np.ndarray) -> None:
    ...
    if H == 3:
        if N >= 4:
            ...
            s[2:-2] = (
                h[0] * c[2:-2] + h[1] * (c[1:-3] + c[3:-1]) + h[2] * (c[0:-4] + c[4:])
            )
    ...
    if H == 4:
        if N >= 6:
            s[0] = (
                h[0] * c[0] + 2.0 * h[1] * c[1] + 2.0 * h[2] * c[2] + 2.0 * h[3] * c[3]
            )
            s[1] = (
                h[0] * c[1]
                + h[1] * (c[0] + c[2])
            ...
            s[4] = (
                h[0] * c[4] + 2.0 * h[1] * c[3] + 2.0 * h[2] * c[2] + 2.0 * h[3] * c[1]
            )
        elif N == 4:
            s[0] = (
                h[0] * c[0] + 2.0 * h[1] * c[1] + 2.0 * h[2] * c[2] + 2.0 * h[3] * c[3]
            ...
            s[3] = (
                h[0] * c[3] + 2.0 * h[1] * c[2] + 2.0 * h[2] * c[1] + 2.0 * h[3] * c[0]
            )
        elif N == 3:
...
def initial_anti_causal_batch(C: np.ndarray, z: float) -> np.ndarray:
    """C: (B, N) -> (B,)"""
    if C.shape[1] < 2:
        return np.zeros(C.shape[0], dtype=C.dtype)
    return (z * C[:, -2] + C[:, -1]) * z / (z * z - 1.0)
...
def symmetric_fir_batch(h: Sequence[float], C: np.ndarray) -> np.ndarray:
    ...
    if H == 4:
        ...
        if N >= 6:
            ...
            S[:, 1] = (
                h0 * C[:, 1] + (h1 + h3) * (C[:, 0] + C[:, 2]) + 2.0 * h2 * C[:, 1]
            )
