...
def generate_3d_rotated_data_and_mask(
    ...
    coords = np.stack(
        [g - c for g, c in zip(grid, custom_center)], axis=0
    )  # Shape: (3, N, N, N)
...
@pytest.mark.parametrize(
    "N, margin, custom_center, angle, k, tolerance, axis",
    [
        # Basic cases
...
def test_3d_rotate_with_center(N, margin, custom_center, angle, k, tolerance, axis):
    ...
    data, data_rotated, mask_original, mask_rotated, coords_flat = (
        generate_3d_rotated_data_and_mask(
            data_shape, custom_center, margin, angle, k, axis
        )
    )
...
def test_rotate_3d_matches_equivalent_scipy_transform(degree):
    rng = np.random.default_rng(20260715 + degree)
    data = rng.standard_normal((13, 12, 11))
    angle = 31.0
    axis = np.array((1.0, 2.0, 3.0))
    axis /= np.linalg.norm(axis)
    center = np.array((4.5, 6.25, 3.75))
    ux, uy, uz = axis
    angle_rad = np.radians(-angle)
