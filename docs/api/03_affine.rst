...
behind a caller-controlled memory cap.  ``AffinePlan.prefilter`` and
``apply_coefficients`` expose deliberate coefficient sharing when one sampled
field is transformed through several compatible geometries.  Equivalent SciPy
