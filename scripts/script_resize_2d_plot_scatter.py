# splineops/scripts/script_resize_2d_plot_scatter.py
# -*- coding: utf-8 -*-
"""
...
def main():
    ...
    def plot_tradeoff_scatter(region: str):
        """
        Plot SNR vs Time, with zoom factor encoded as color (scatter)
        and method encoded as line+marker color/shape (legend).
        """
        if region == "down":
            title_suffix = " (downsampling, 0 < z < 1)"
            mask_fn = lambda z: z < 1.0
        ...
        # One color per method (using matplotlib's default color cycle)
        ...
        for idx_name, name in enumerate(results.keys()):
            color_for[name] = base_colors[idx_name % len(base_colors)]
        ...
        for name, data in results.items():
            ...
            s_arr = np.asarray(data["snr"], dtype=float)

            # Keep only this zoom regime + finite values
            mask = mask_fn(z_arr) & np.isfinite(t_arr) & np.isfinite(s_arr)
            if not mask.any():
                continue
            ...
            # Sort by zoom so each method's curve is ordered
            order = np.argsort(z_m)
            z_m = z_m[order]
            t_m = t_m[order]
            s_m = s_m[order]

            # Scatter: color = zoom (for colorbar), NO label (kept out of legend)
            ...
            # Thick line + marker: color = method, used in legend
            plt.plot(
                t_m,
                s_m,
                color=color_for[name],
                marker=marker_for.get(name, "o"),
                linewidth=TRADEOFF_LINEWIDTH,
        ...
        if any_points:
            plt.xscale("log")
            plt.xlabel(
                f"Time (s, log scale)  [forward + backward, avg of {args.repeats} runs]",
                fontsize=PLOT_LABEL_FONTSIZE,
            )
            plt.ylabel(
                "SNR (dB)  [original vs recovered]",
                fontsize=PLOT_LABEL_FONTSIZE,
            )
            plt.title(
                f"SNR vs Time (colored by zoom){title_suffix}  "
                f"(H×W = {H}×{W}, dtype={DTYPE_NAME}, degree={degree_label})",
                fontsize=PLOT_TITLE_FONTSIZE,
            )
            plt.xticks(fontsize=PLOT_TICK_FONTSIZE)
            ...
            # Legend: one entry per method (from the lines)
            plt.legend(fontsize=PLOT_LEGEND_FONTSIZE)

            # Colorbar: zoom factor
            if last_scatter is not None:
                cbar = plt.colorbar(last_scatter)
                cbar.set_label("Zoom factor z", fontsize=PLOT_LABEL_FONTSIZE)
    ...
    if args.which in ("both", "down"):
        plot_tradeoff_scatter("down")
    if args.which in ("both", "up"):
        plot_tradeoff_scatter("up")
