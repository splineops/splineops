...
def _u8_to_gray01(u8_rgb: np.ndarray) -> np.ndarray:
    ...
    return (0.2989 * u[..., 0] + 0.5870 * u[..., 1] + 0.1140 * u[..., 2]).astype(
        np.float32
    )
...
def _resize_rgb_splineops(img01: np.ndarray, z: float, *, method: str) -> np.ndarray:
    ...
    chs = [
        sp_resize(img01[..., c], zoom_factors=zoom_hw, method=method) for c in range(3)
    ]
...
def rt_splineops_aa(orig01: np.ndarray, z: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    rec_ch = [
        sp_resize(down01[..., c], output_size=(H0, W0), method="cubic-antialiasing")
        for c in range(3)
    ]
...
def rt_torch_bicubic(orig01: np.ndarray, z: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    x = (
        torch.from_numpy(orig01.astype(np.float32, copy=False))
        .permute(2, 0, 1)
        .unsqueeze(0)
    )
    ...
    try:
        y = F.interpolate(
            x, size=(H1, W1), mode="bicubic", align_corners=False, antialias=False
        )
        y2 = F.interpolate(
            y, size=(H0, W0), mode="bicubic", align_corners=False, antialias=False
        )
    except TypeError:
...
def get_competitor_rt() -> (
    tuple[str, Callable[[np.ndarray, float], tuple[np.ndarray, np.ndarray]]]
):
    if _HAS_TORCH:
    ...
    raise RuntimeError(
        "Neither PyTorch nor SciPy is available for the competitor backend."
    )
...
def make_benchmark_animation(
    ...
    *,
    ...
    for z in zoom_values:
        ...
        max_abs = max(
            max_abs, float(np.max(np.abs(_u8_to_gray01(rec_a) - orig_gray01)))
        )
        max_abs = max(
            max_abs, float(np.max(np.abs(_u8_to_gray01(rec_b) - orig_gray01)))
        )
    ...
    def _snr_db(rec_gray01: np.ndarray) -> float:
        den = float(
            np.sum(
                (orig_gray01.astype(np.float64) - rec_gray01.astype(np.float64)) ** 2
            )
        )
        if den == 0.0:
            return float("inf")
    ...
    ax_leg_host.text(
        ...
        0.05,
        "-1",
        transform=ax_leg_host.transAxes,
        fontsize=9,
        va="bottom",
        ha="left",
    )
    ...
    im_err_a = ax_err_a.imshow(
        diff_norm_u8_from_gray(rec_cmp_g0), cmap="gray", vmin=0, vmax=255
    )
    im_err_b = ax_err_b.imshow(
        diff_norm_u8_from_gray(rec_so_g0), cmap="gray", vmin=0, vmax=255
    )
