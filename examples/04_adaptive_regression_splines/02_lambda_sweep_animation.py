...
def create_lambda_sweep_animation(lambdas: np.ndarray, interval: int = 700):
    ...
    den_line, = ax.plot(x, sols[0]["y_d"], "x", label="Denoised", markersize=8)
    spline_line, = ax.plot(sols[0]["t"], sols[0]["y_s"], label="Sparsest")
    ...
    def animate(i: int):
        ...
        return den_line, spline_line, knot_scatter, title
