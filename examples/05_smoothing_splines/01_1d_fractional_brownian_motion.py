...
print(
    f"mSNR improvement of the denoised sequence                 is {SNR - SNR0:.2f} dB."
)
print(
    f"mSNR improvement of the denoised and oversampled sequence is {SNRm - SNR0:.2f} dB."
)
...
plt.plot(
    t[: len(t) // 2], y_noisy[: len(y_noisy) // 2], "k+:", label="Noisy fBm samples"
)
...
plt.plot(
    ...
    y_est[: len(y_est) // 2],
    "r",
    label="Non-stationary estimation",
)
plt.legend()
