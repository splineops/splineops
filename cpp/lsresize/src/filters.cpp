...
template <typename Scalar>
static void add_average_scaled_colmajor(
  std::vector<Scalar>& c,
  int B,
  int N,
  const std::vector<Scalar>& average,
  Scalar scale)
{
  ...
  for (int n = 0; n < N; ++n) {
    ...
    for (int b = 0; b < B; ++b) {
      const size_t bi = static_cast<size_t>(b);
      row[bi] = (row[bi] + average[bi]) * scale;
    }
  }
}
...
template <typename Scalar>
static void diff_as_colmajor_add_average_scaled(
  std::vector<Scalar>& c,
  int B,
  int N,
  const std::vector<Scalar>& average,
  Scalar scale)
{
  ...
  if (N == 1) {
    ...
    for (int b = 0; b < B; ++b) {
      const size_t bi = static_cast<size_t>(b);
      first[bi] = (first[bi] * static_cast<Scalar>(2) + average[bi]) * scale;
    }
    return;
  }
  ...
  for (int n = N - 1; n > 0; --n) {
    ...
    for (int b = 0; b < B; ++b) {
      const size_t bi = static_cast<size_t>(b);
      cur[bi] = (cur[bi] - prev[bi] + average[bi]) * scale;
    }
  }
...
template <typename Scalar>
static void diff_sa_as_colmajor_pair_add_average_scaled(
  std::vector<Scalar>& c,
  int B,
  int N,
  const std::vector<Scalar>& average,
  ...
  std::vector<Scalar>& work)
{
...

  {
    ...
    for (int b = 0; b < B; ++b) {
      const size_t bi = static_cast<size_t>(b);
      const Scalar d0 = work[bi] - row1[bi];
      row0[bi] = (d0 * static_cast<Scalar>(2) + average[bi]) * scale;
    }
  }
  ...
  for (int n = 1; n + 1 < N; ++n) {
    ...
    for (int b = 0; b < B; ++b) {
      ...
      const Scalar orig_cur = cur[bi];
      cur[bi] =
          ((orig_cur - next[bi]) - (work[bi] - orig_cur) + average[bi]) *
          scale;
      work[bi] = orig_cur;
    }
  }

  {
    ...
    for (int b = 0; b < B; ++b) {
      ...
      const Scalar orig_last = last[bi];
      last[bi] =
          ((orig_last - work[bi]) - (work[bi] - orig_last) + average[bi]) *
          scale;
    }
  }
}
...
template <typename Scalar>
static void do_diff_colmajor_add_average_scaled_impl(
  std::vector<Scalar>& c,
  int B,
  int N,
  int nb,
  ...
  std::vector<Scalar>& work)
{
  if (B <= 0 || N <= 0) return;
  if (nb <= 0) {
    add_average_scaled_colmajor(c, B, N, average, scale);
    return;
  }
  if (nb == 1) {
    diff_as_colmajor_add_average_scaled(c, B, N, average, scale);
    return;
  }
  if (nb == 2) {
    diff_sa_as_colmajor_pair_add_average_scaled(
        c, B, N, average, scale, work);
    return;
  }
  if (nb == 3) {
    diff_as_colmajor_plain(c, B, N);
...
void do_diff_colmajor_add_average_scaled(
  ...
  std::vector<double>& work)
{
  do_diff_colmajor_add_average_scaled_impl(
      c, B, N, nb, average, scale, work);
}
