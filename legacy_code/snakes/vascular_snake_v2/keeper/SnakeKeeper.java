...
public class SnakeKeeper implements ROIListener, Observer, SequenceListener {
	...
	private ArrayList<Thread> runningThreadList_ = new ArrayList<Thread>();

	private OptimizationThread optimizationThread_ = null;

	private Lock isOptimizingLock_ = new ReentrantLock();
	private Lock snakeLock_ = new ReentrantLock();
	private Lock roiLock_ = new ReentrantLock();
	private Lock screenUpdateLock = new ReentrantLock();

	private String ID = null;

	private Condition updateCondition = screenUpdateLock.newCondition();

	/** The ROI managing control points. */
	protected Snake3DROI roiNodes_ = null;
	...
	private boolean isRefreshDuringOptimization_ = Settings.REFRESH_SCREEN_DEFAULT;
	private boolean isRefreshDrag_ = Settings.REFRESH_SNAKE_DRAG_DEFAULT;
	...
	/** Update thread. */
	private Thread updateThread_ = new Thread() {
		@Override
		public void run() {
	...
	/** Default constructor. */
	public SnakeKeeper(Sequence sequence, CylinderSnake snake,
			BIGVascular mainPlugin) {
		if (sequence == null) {
		...
		roiLock_.lock();
		roiNodes_ = new Snake3DROI(snake_, this);
	...
	public CylinderSnakeParameters getESnakeParameters() {
	...
	public void setDisplaySettings(final DisplaySettings displaySettings) {
		...
		try {
			...
			roiNodes_.setRefreshDrag(displaySettings.refreshDrag());
	...
	public void setEditingMode(final SnakeEditMode editingMode) {
		roiLock_.lock();
		try {
	...
	public void setSnakeEditMode(final SnakeEditMode editingMode) {
		roiLock_.lock();
		try {
			roiNodes_.setEditMode(editingMode);
		} finally {
	...
	public void setSnakeParameters(final CylinderSnakeParameters snakeParameters) {
		Thread thr = new Thread() {
			@Override
			public void run() {
			...
			if (isOptimizingLock_.tryLock()) {
				try {
					...
					if (isRefreshDuringOptimization_) {
						...
						isOptimizing_ = false;
						snake_.reviveSnake();
						optimizer_.deleteObserver(SnakeKeeper.this);
