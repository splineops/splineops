...
public class HystStrip2 extends Plugin{
	...
	public HystStrip2(byte[][][] input, double[][][][] orientationAllInOne){
		inputAllInOne = input;
	...
	public void process(){
		...
		for (int z=1;z<nz-1;z++) {
			...
			if (val > 0){
				...
				double[] p1 = {x, y, z};				
				double[][] p2Minus = new double[7][3];
				double[][] p2Plus = new double[7][3];
				...
				for (int i=1; i<DIST; i++){							
					...
					if (keep){						
						...
						if (output[x-xRound][y-yRound][z-zRound]!=0){
							double[] p = {x-xRound, y-yRound, z-zRound};	
							p2Minus[0] = p;
						}
						if (z+zRound < nz && y+yRound < ny && x+xRound < nx)
						if (z+zRound >= 0  && y+yRound >= 0 && x+xRound >= 0)
						...
						if (output[x-xRound2][y-yRound2][z-zRound2]!=0){
							double[] p = {x-xRound2, y-yRound2, z-zRound2};	
							p2Minus[1] = p;
						}
						if (z+zRound2 < nz && y+yRound2 < ny && x+xRound2 < nx)
						if (z+zRound2 >= 0  && y+yRound2 >= 0 && x+xRound2 >= 0)
		...
		if (Const.GUI2_chkSteps)
			showZ("Step 5: Skull stripping 1");
