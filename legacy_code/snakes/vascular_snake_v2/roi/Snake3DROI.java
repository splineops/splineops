 ...
 ******************************************************************************/
package plugins.big.vascular.roi;
...
import loci.poi.hssf.util.HSSFColor.BLUE;
...
import vtk.vtkSphereRepresentation;
 ...
 */
public class Snake3DROI extends ROI3D implements Anchor2DListener {

	/** Snake associated to the ROI. */
	private CylinderSnake snake_ = null;
	...
	// TODO write javadoc
	private final ArrayList<Anchor3D> nodes_ = new ArrayList<Anchor3D>();
	...
	/** Indicate if the snake must redraw on drag **/
	private boolean refreshDrag_;
	...
	// TODO write javadoc
	private float strokeMultiplier_ = Settings.STROKE_THICKNESS_DEFAULT;
	/** Controls the number of scales shown on the screen. */
	private int scaleSubsamplingFactor_ = 4;
	// TODO write javadoc
	private int displayRange_ = Settings.DEPTH_TRANSPARENCY_DEFAULT;
	...
	/** Default constructor. */
	public Snake3DROI(CylinderSnake snake, SnakeKeeper keeper) {
		super();
		isEditable_ = true;
		snake_ = snake;
		keeper_ = keeper;
	...
	public void setEditMode(SnakeEditMode editMode) {
		...
		if (editMode == SnakeEditMode.MOVE_SNAKE) {
			try {
				nodesLock_.lock();
				for (Anchor3D a : nodes_) {
					a.setSelected(true);
	...
	public void setPixelSize(double pixelSizeX, double pixelSizeY,
		...
		try {
			...
			if (pixelSizeX != pixelSizeX_ || pixelSizeY != pixelSizeY_
				...
				if (painter != null && this.painter instanceof Snake3DPainter) {
	...
	private void addControlPoint(double x, double y, double z) {
		Anchor3D point = new Anchor3D(x, y, z, DEFAULT_NORMAL_COLOR,
	...
	private class Snake3DPainter extends ROIPainter {
		 ...
		 * point.
		 */
		private SnakeInteraction snakeInteraction_ = SnakeInteraction.NOTHING;
		...
		public Snake3DPainter(double pixelSizeX, double pixelSizeY,
				double pixelSizeZ) {
			System.out.println("Snake3DROI: Snake3DPainter");
			...
			if (painter3Dinitialized) {
				for (Viewer v : sequence.getViewers()) {
					...
					if (canvas instanceof Canvas3D) {
						...
						for (vtkActor actor : nodesActors) {
							renderer.RemoveActor(actor);
						}
					}
				}
		...
		public void mouseReleased(MouseEvent e, Point5D.Double imagePoint,
			...
			if (canvas instanceof IcyCanvas2D) {
				if (!isEditable_) {
					...
					Snake3DROI.this.beginUpdate();
					try {
						if (Snake3DROI.this.sectionContains(new Point2D.Double(
								imagePoint.getX(), imagePoint.getY()))) {
							snakeInteraction_ = SnakeInteraction.SCALE;
						} else {
							snakeInteraction_ = SnakeInteraction.NOTHING;
						}
						if (snakeInteraction_ == SnakeInteraction.SCALE
								&& imagePoint.equals(pressedImagePoint_)) {
							keeper_.activateSnake();
						}
					} finally {
						Snake3DROI.this.endUpdate();
					}
				}
			} else if (canvas instanceof IcyCanvas3D) {
				Canvas3D canvas3D = (Canvas3D) canvas;

				if (!e.isConsumed() && snakeMoved) {
					for (int i = 0; i < Snake3DROI.this.nodes_.size(); i++) {
		...
		private void mouseMoveDefault(MouseEvent e, Point5D.Double imagePoint,
			...
			try {
				if (canvas instanceof IcyCanvas2D) {
					snakeInteraction_ = getSnakeMouseInteraction(imagePoint,
							canvas);

					Anchor3D selectedControlPoint = null;
					for (Anchor3D pt : nodes_) {
						if (pt.isOver(canvas, imagePoint.getX(),
								imagePoint.getY())) {
					...
					// if mouse is over a point, it is colored in green
		...
		private void mousePressedDefault(MouseEvent e,
			...
			try {
				if (canvas instanceof IcyCanvas2D) {
					...
					// if mouse is over a point select the point otherwise
					// select
					// the all snake
					Canvas3D canvas3D = (Canvas3D) canvas;
					if (pickControlPoint(canvas3D, e) != null) {
			...
			try {
				if (canvas instanceof IcyCanvas2D) {
					...
					case SCALE:
						...
						double[] pos = Snake3DROI.this.getControlPoint(0);
						firstControlPointInitPosition_ = new Point2D.Double(
								pos[0], pos[1]);
						e.consume();
						break;
					}
			...
			try {
				if (canvas instanceof IcyCanvas2D) {
					...
					case SCALE:
						...
						((Canvas2D) canvas).getViewComponent().setCursor(
								dragSnakeCursor_);
						double dx = imagePoint.getX()
								- pressedImagePoint_.getX();
						double dy = imagePoint.getY()
								- pressedImagePoint_.getY();
						pressedImagePoint_ = imagePoint;
						Snake3DROI.this.translate2D(dx, dy, 0);
						break;
					}
		...
		private void mouseDragDilate(MouseEvent e, Point5D.Double imagePoint,
			...
			try {
				...
				if (pressedImagePoint_ != null && snakeMassCenter_ != null
					...
					System.out.println("snakeInteractoin_ = "
							+ snakeInteraction_);
					switch (snakeInteraction_) {
					case NOTHING:
						// e.consume();
		...
		private SnakeInteraction getSnakeMouseInteraction(
				Point5D.Double imagePoint, IcyCanvas canvas) {
			...
			if (Snake3DROI.this.sectionContains(new Point2D.Double(imagePoint
					.getX(), imagePoint.getY()))) {
				return SnakeInteraction.SCALE;
			}

			return SnakeInteraction.NOTHING;
		...
		private void setPositionFromMouseEvent(Canvas3D canvas3D, MouseEvent e) {
			...
			Snake3DROI.this.nodes_.get(indexOfControlPoint(controlPoint))
					.setPosition(worldPos[0], worldPos[1], worldPos[2]);
		}
		...
		private void setPositionFromVector(Canvas3D canvas3D, double[] vector,
			...
			try {
				...
				if (isEditable_) {
					...
					// get the z depth of the control point
					double[] point = {
							getInDisplayCoordinates(canvas3D, position, 0),
							getInDisplayCoordinates(canvas3D, position, 1),
							getInDisplayCoordinates(canvas3D, position, 2) };

					// invert the y and create the mouse position point
					double[] pos = { point[0] + vector[0],
							point[1] - vector[1], point[2] };

					// translate this point in world coordinate
					double[] worldPos = transformDisplayPointToWorld(canvas3D,
							pos);
					worldPos = pointToNodeScale(worldPos, this.pixelSizeX,
							this.pixelSizeY, this.pixelSizeZ);

					// modify the control point position in nodes_
					Snake3DROI.this.nodes_.get(
							indexOfControlPoint(controlPoint)).setPosition(
	...
	public void setRefreshDrag(boolean refreshDrag) {
