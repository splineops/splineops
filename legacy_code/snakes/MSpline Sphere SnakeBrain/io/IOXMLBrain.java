 ...
 ******************************************************************************/
package plugins.big.bigsnake3d.io;
...
public class IOXMLBrain {
	 ...
	 */
	public static void saveSnakesToXML(ImageLUTContainerMSBrain imageLUT)
			throws Exception {
		if (imageLUT == null) {
	 ...
	 */
	public static ArrayList<SnakeKeeperMSBrain> loadSnakesFromXML(
			ImageLUTContainerMSBrain imageLUT, BIGSnake3DMSBrain mainPlugin) throws Exception {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			ArrayList<SnakeKeeperMSBrain> loadedKeepers = new ArrayList<SnakeKeeperMSBrain>();
			...
			if (nodeROIs != null) {
				ArrayList<Snake3DNode[]> controlPointsList = loadSnakeNodesFromXML(nodeROIs);
				ArrayList<SphereSnakeParametersMSBrain> snakeParametersList = loadSnakeParametersFromXML(nodeROIs);

				int detectedSnakes = controlPointsList.size();
				for (int i = 0; i < detectedSnakes; i++) {
					SphereSnakeMSBrain snake = new SphereSnakeMSBrain(imageLUT,
							snakeParametersList.get(i));
					snake.setNodes(controlPointsList.get(i));
					loadedKeepers.add(new SnakeKeeperMSBrain(sequence, snake,
							mainPlugin));
				}
	...
	private static void addMetaDataToXML(Node node, ImageLUTContainerMSBrain imageLUT)
			throws Exception {
		if (node == null) {
		...
		if (nodeMeta != null) {
			...
			XMLUtil.setElementDoubleValue(nodeMeta, ImageLUTContainerMSBrain.ID_SIGMA,
					imageLUT.getSigma());
			XMLUtil.setElementDoubleValue(nodeMeta,
					ImageLUTContainerMSBrain.ID_ACTIVECHANNEL,
	...
	private static void loadMetaDataFromXML(Node node,
			ImageLUTContainerMSBrain imageLUT) throws Exception {
		...
		if (nodeMeta != null) {
			imageLUT.setChannelNumber(XMLUtil.getElementIntValue(nodeMeta,
					ImageLUTContainerMSBrain.ID_ACTIVECHANNEL, 0));
			imageLUT.setSigma(XMLUtil.getElementDoubleValue(nodeMeta,
					ImageLUTContainerMSBrain.ID_SIGMA, 10d));
		}
		...
		if (nodesROI != null) {
			for (Node n : nodesROI) {
				final Node snakeParametersNode = XMLUtil.getElement(n,
						ROI3DSnakeMSBrain.ID_SNAKE_PARAMETERS);
				final Node controlPointsNode = XMLUtil.getElement(
						snakeParametersNode, SphereSnakeMSBrain.ID_CONTROL_POINTS);
				final ArrayList<Node> controlPointNodeList = XMLUtil
						.getChildren(controlPointsNode, SphereSnakeMSBrain.ID_CONTROL_POINT);
	...
	private static ArrayList<SphereSnakeParametersMSBrain> loadSnakeParametersFromXML(
		...
		final ArrayList<Node> nodesROI = XMLUtil.getChildren(nodeROIs, ID_ROI);
		ArrayList<SphereSnakeParametersMSBrain> snakeParametersList = new ArrayList<SphereSnakeParametersMSBrain>();
		if (nodesROI != null) {
			for (Node n : nodesROI) {
				final Node snakeParametersNode = XMLUtil.getElement(n,
						ROI3DSnakeMSBrain.ID_SNAKE_PARAMETERS);
