 ...
 */
public class ImageSettingsPaneMSBrain extends CollapsiblePanel implements
	 ...
	 */
	private HashMap<Sequence, ImageLUTContainerMSBrain> imageLUTs_ = null;
	...
	/** Constructor. */
	public ImageSettingsPaneMSBrain(String title,
			HashMap<Sequence, ImageLUTContainerMSBrain> imageLUTs) {
		super(title);
		...
		gaussianBlurSpinner_.setModel(new SpinnerNumberModel(new Double(10),
	...
	public void sequenceFocused(Sequence sequence) {
		...
		if (sequence != null) {
			...
			if (imageLUTs_.containsKey(sequence)) {
				ImageLUTContainerMSBrain imageLUT = imageLUTs_.get(sequence);
				selectedChannelComboBox_.setSelectedIndex(imageLUT
						.getChannelNumber());
				gaussianBlurSpinner_.setValue(imageLUT.getSigma());
			} else {
				ImageLUTContainerMSBrain imageLUT = new ImageLUTContainerMSBrain();
	...
	public void stateChanged(ChangeEvent e) {
		if (e.getSource() == gaussianBlurSpinner_) {
			if (imageLUTs_.containsKey(activeSequence_)) {
				ImageLUTContainerMSBrain imageLUT = imageLUTs_.get(activeSequence_);
				imageLUT.setSigma((Double) gaussianBlurSpinner_.getValue());
			} else {
				ImageLUTContainerMSBrain imageLUT = new ImageLUTContainerMSBrain();
				imageLUT.setSequence(activeSequence_);
