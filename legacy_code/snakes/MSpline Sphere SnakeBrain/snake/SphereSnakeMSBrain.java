...
//import plugins.big.bigbrain.utils.Const;
...
/**
 * Three-dimensional Milano-spline (MSpline) sphere snake.
 * 
 ...
 */
public class SphereSnakeMSBrain implements Snake3D {
	...
	private SphereSnakeEnergyTypeMSBrain energyType_ = SphereSnakeEnergyTypeMSBrain.REGION;
	/** Indicates the type of features to detect (bright or dark). **/
	private SphereSnakeTargetTypeMSBrain detectType_ = SphereSnakeTargetTypeMSBrain.BRIGHT;
	/**
	...
	private ImageLUTContainerMSBrain imageLUTs_ = null;
	...
	/** Default constructor. */
	public SphereSnakeMSBrain(ImageLUTContainerMSBrain imageLUTs,
			SphereSnakeParametersMSBrain parameters) {
		if (imageLUTs == null) {
	...
	public double energy() {
		...
		if (life_ % 5000 == 0)
			System.out.println("life_ = " + life_);
		...
		if (detectType_ == SphereSnakeTargetTypeMSBrain.DARK) {
			return -E + boundaryEnergy + gamma_ * stiffnessEnergy;
		} else if (detectType_ == SphereSnakeTargetTypeMSBrain.BRIGHT) {
			return E + boundaryEnergy + gamma_ * stiffnessEnergy;
		} else {
	...
	public void initialize() {
		...
		updateArea();
		if (energyType_ == SphereSnakeEnergyTypeMSBrain.REGION
				|| energyType_ == SphereSnakeEnergyTypeMSBrain.MIXTURE) {
			updateVolume();
		}
		if (energyType_ == SphereSnakeEnergyTypeMSBrain.REGION
				|| energyType_ == SphereSnakeEnergyTypeMSBrain.MIXTURE) {
	...
	/**
	 * Saves the snake-defining parameters in an XML file. , should overide a
	 * function in Snake3D
	 */
	...
	public SphereSnakeParametersMSBrain getSnakeParameters() {
		return new SphereSnakeParametersMSBrain(maxLife_, M_, alpha_, gamma_,
	...
	private void buildLUTs() {
		...
		for (int i = 0; i < NR2_; i++) {
			...
			bSpline2LUT_[i] = BSplineBasis.MSpline(currentVal, PIM_);
			bSpline1DerivativeLUT_[i] = BSplineBasis.MSpline_Prime(currentVal,
					PI2M_);
			bSpline2DerivativeLUT_[i] = BSplineBasis.MSpline_Prime(currentVal,
					PIM_);
			bSpline1SecondDerivativeLUT_[i] = BSplineBasis.MSpline_PrimePrime(
					currentVal, PI2M_);
			bSpline2SecondDerivativeLUT_[i] = BSplineBasis.MSpline_PrimePrime(
					currentVal, PIM_);
		}

		for (int i = 0; i < M_; i++) {
			double theta = PI2M_ * i;
			cosLUT_[i] = Math.cos(theta);
			sinLUT_[i] = Math.sin(theta);
		}
	}
	...
	private void initializeDefaultShape() {
		...
		// START: initialization for brain snake
		 ...
		 * double y0 = imageLUTs_.getImageHeight() / 2.7;
		 * //imageLUTs_.getImageHeight() / 2.4;GB_1, MB_1
		 * //imageLUTs_.getImageHeight() / 3; DS_1 //imageLUTs_.getImageHeight()
		 * / 2.5 DR_1; // 90 DP_1, CG_1 double z0 = imageLUTs_.getImageDepth() /
		 * 2.0; // 80 DP_1
		 */
		...
		for (int k = 0; k < M_; k++) {
			for (int l = 1; l <= M_ - 1; l++) {
				...
				coef_[k + (l - 1) * M_] = new Snake3DNode(x0 + rX0 * scale2M
						* Math.sin(theta) * cosLUT_[k], y0 + rY0 * scale2M
						* Math.sin(theta) * sinLUT_[k], z0 + rZ0 * scale2M
						* Math.cos(theta));

				/*
				 * coef_[k + (l - 1) * M_] = new Snake3DNode(x0 + 1.9*r *
				 * scale2M Math.sin(theta) * cc_[k], y0 + 0.9*r * scale2M
				...
				/*
				 * coef_[k + (l - 1) * M_] = new Snake3DNode(x0 + 1.9*r *
				 * scale2M Math.sin(theta) * cc_[k], y0 + 1.0*r * scale2M
		...
		/*
		 * // north pole coef_[M_ * (M_ - 1)] = new Snake3DNode(x0, y0, z0 + r);
		 * // south pole coef_[M_ * (M_ - 1) + 1] = new Snake3DNode(x0, y0, z0 -
		 * r);
		 */
		...
		// END: initialization for brain snake

		/*
		 * coef_ = new Snake3DNode[M_ * (M_ - 1) + 6];
		 * 
		 * double r = Math.min( Math.min(imageLUTs_.getImageWidth() / 5.0,
		 * imageLUTs_.getImageHeight() / 5.0), imageLUTs_.getImageDepth() /
		 * 5.0);
		 * 
		 * double x0 = imageLUTs_.getImageWidth() / 2.0; double y0 =
		 * imageLUTs_.getImageHeight() / 2.0; double z0 =
		 * imageLUTs_.getImageDepth() / 2.0;
		 * 
		 * // double scale2M = 2.0 * (1.0 - Math.cos(PIM_)) / (Math.cos(PIM_ /
		 * 2) - // Math.cos(3.0 * PIM_ / 2));
		 * 
		 * // START: factors for snake initialization for 3D cells (SPL paper)
		 * double f = 0.8; double axy = f * 5; //3.14; double az = f * 0.8; //1;
		 * 
		 * // END: factors for snake initialization for 3D cells (SPL paper)
		 * 
		 * for (int k = 0; k < M_; k++) { for (int l = 1; l <= M_ - 1; l++) {
		 * double theta = PIM_ * l; coef_[k + (l - 1) * M_] = new Snake3DNode(x0
		 * + axy * r Math.sin(theta) * cosLUT_[k], y0 + axy * r Math.sin(theta)
		 * * sinLUT_[k], z0 + az * r Math.cos(theta)); } } // north pole
		 * coef_[M_ * (M_ - 1)] = new Snake3DNode(x0, y0, z0 + az * r); // south
		 * pole coef_[M_ * (M_ - 1) + 1] = new Snake3DNode(x0, y0, z0 - az * r);
		 * // north tangent plane coef_[M_ * (M_ - 1) + 2] = new Snake3DNode(x0
		 * + axy * Math.PI * r, y0, z0 + az * r); coef_[M_ * (M_ - 1) + 3] = new
		 * Snake3DNode(x0, y0 + axy * Math.PI * r, z0 + az * r); // south
		 * tangent plane coef_[M_ * (M_ - 1) + 4] = new Snake3DNode(x0 + axy *
		 * Math.PI * r, y0, z0 - az * r); coef_[M_ * (M_ - 1) + 5] = new
		 * Snake3DNode(x0, y0 + axy * Math.PI * r, z0 - az * r);
		 */
	...
	// ------------------------
	/*
	 * function only used for brain segmentation initialization
	 */
	private double[] getX0Y0RxRyRz() {
		...
		if (Const.segmentBrainMP2RAGE) {
			...
			imageLUTs_.brainSequence = Icy.getMainInterface()
					.getSequences("binaryImage").get(0);
		}
		*/

		int nx = imageLUTs_.getImageWidth();  //imageLUTs_.brainSequence.getSizeX();
		int ny = imageLUTs_.getImageHeight(); //imageLUTs_.brainSequence.getSizeY();
		int nz = imageLUTs_.getImageDepth();  //imageLUTs_.brainSequence.getSizeZ();
		...
		double[][] image = imageLUTs_.getImageDataArray();
		System.out.println("image = " + image);
		System.out.println("image[0].length = " + image[0].length + ", image[0].length = " + image[0].length);
		
		double val;
		for (int x = 0; x < nx; x++)
			for (int y = 0; y < ny; y++) {
				val = image[z0][x + nx * y];  //imageLUTs_.brainSequence.getData(0, z0, 0, y, x);
				// if (val != 0){
				if (val > 0) {
					if (x < minX)
						minX = x;
		...
		for (int x = 0; x < nx; x++)
			for (int z = 0; z < nz; z++) {
				val = image[z][x + nx * y0];//imageLUTs_.brainSequence.getData(0, z, 0, y0, x);
				if (val > 0) {
					if (z < minZ)
						minZ = z;
					if (z > maxZ)
		...
		double[] x0y0RxRyRz = { (0.52 * (minX + maxX)), (0.4 * (minY + maxY)),
				0.4 * (maxX - minX), 0.25 * (maxY - minY), 0.4 * (maxZ - minZ) };
	...
	private void updateSnakeSkin() {
		...
		for (int i = 0; i < MR_; i++) {
			for (int j = 0; j <= MR_; j++) {
				...
				zPosVal = 0;
				for (int l = 1; l <= M_ - 1; l++) {
					// index_j = 2 * j + nSamplesPerSegment_ * (3 - 2 * l);
					index_j = 2 * j + nSamplesPerSegment_ * (4 - 2 * l);

					if (index_j >= 0 && index_j < NR2_) {
						aux = bSpline2LUT_[index_j];
						for (int k = 0; k < M_; k++) {
							// index_i = (2 * i + nSamplesPerSegment_ * (3 - 2 *
							// k) + MR2_) % MR2_;
				...
				// double scal = 1.0 / (M_ * bSpline2DerivativeLUT_[5 *
				// nSamplesPerSegment_]);
				double scal = 1.0 / (M_ * bSpline2DerivativeLUT_[6 * nSamplesPerSegment_]);
				...
				// l = -1
				// index_j = 2 * j + nSamplesPerSegment_ * 5;
				index_j = 2 * j + nSamplesPerSegment_ * 6;

				if (index_j >= 0 && index_j < NR2_) {
					aux = bSpline2LUT_[index_j];
					for (int k = 0; k < M_; k++) {
						// index_i = (2 * i + nSamplesPerSegment_ * (3 - 2 * k)
						// + MR2_) % MR2_;
						index_i = (2 * i + nSamplesPerSegment_ * (4 - 2 * k) + MR2_)
								% MR2_;
				...
				// l = 0
				// index_j = 2 * j + nSamplesPerSegment_ * 3;
				index_j = 2 * j + nSamplesPerSegment_ * 4;
				...
				if (index_j >= 0 && index_j < NR2_) {
					...
					for (int k = 0; k < M_; k++) {
						...
						if (index_i >= 0 && index_i < NR2_) {
							...
							/*
							 * double ckminus1x = (coef_[k].x + scal (cosLUT_[k]
							 * * NorthV1x + sinLUT_[k] NorthV2x)); double
							 * ckminus1y = (coef_[k].y + scal (cosLUT_[k] *
							 * NorthV1y + sinLUT_[k] NorthV2y)); double
							 * ckminus1z = (coef_[k].z + scal (cosLUT_[k] *
							 * NorthV1z + sinLUT_[k] NorthV2z));
							 */

							// compute c[k,0]
							// double ck0x = (coef_[M_ * (M_ - 1)].x -
							// bSpline2LUT_[5 * nSamplesPerSegment_] *
							// (ckminus1x + coef_[k].x)) / bSpline2LUT_[3 *
							// nSamplesPerSegment_];
							double ck0x = coef_[M_ * (M_ - 1)].x;

							// double ck0y = (coef_[M_ * (M_ - 1)].y -
							// bSpline2LUT_[5 * nSamplesPerSegment_] *
							// (ckminus1y + coef_[k].y)) / bSpline2LUT_[3 *
							// nSamplesPerSegment_];
							double ck0y = coef_[M_ * (M_ - 1)].y;

							// double ck0z = (coef_[M_ * (M_ - 1)].z -
							// bSpline2LUT_[5 * nSamplesPerSegment_] *
							// (ckminus1z + coef_[k].z)) / bSpline2LUT_[3 *
							// nSamplesPerSegment_];
							double ck0z = coef_[M_ * (M_ - 1)].z;
				...
				// l = M+1
				// index_j = 2 * j + nSamplesPerSegment_ * (1 - 2 * M_);
				index_j = 2 * j + nSamplesPerSegment_ * (2 - 2 * M_);
				...
				// l = M
				// index_j = 2 * j + nSamplesPerSegment_ * (3 - 2 * M_);
				index_j = 2 * j + nSamplesPerSegment_ * (4 - 2 * M_);
				...
				if (index_j >= 0 && index_j < NR2_) {
					...
					for (int k = 0; k < M_; k++) {
						...
						if (index_i >= 0 && index_i < NR2_) {
							...
							/*
							 * double ckMplusx = (coef_[k + (M_ - 2) * M_].x +
							 * scal (cosLUT_[k] * SouthV1x + sinLUT_[k]
							 * SouthV2x)); double ckMplusy = (coef_[k + (M_ - 2)
							 * * M_].y + scal (cosLUT_[k] * SouthV1y +
							 * sinLUT_[k] SouthV2y)); double ckMplusz = (coef_[k
							 * + (M_ - 2) * M_].z + scal (cosLUT_[k] * SouthV1z
							 * + sinLUT_[k] SouthV2z));
							 */

							// compute c[k,M]
							// double ckMx = (coef_[M_ * (M_ - 1) + 1].x -
							// bSpline2LUT_[5 * nSamplesPerSegment_] * (ckMplusx
							// + coef_[k + (M_ - 2) * M_].x)) / bSpline2LUT_[3 *
							// nSamplesPerSegment_];
							double ckMx = coef_[M_ * (M_ - 1) + 1].x;

							// double ckMy = (coef_[M_ * (M_ - 1) + 1].y -
							// bSpline2LUT_[5 * nSamplesPerSegment_] * (ckMplusy
							// + coef_[k + (M_ - 2) * M_].y)) / bSpline2LUT_[3 *
							// nSamplesPerSegment_];
							double ckMy = coef_[M_ * (M_ - 1) + 1].y;

							// double ckMz = (coef_[M_ * (M_ - 1) + 1].z -
							// bSpline2LUT_[5 * nSamplesPerSegment_] * (ckMplusz
							// + coef_[k + (M_ - 2) * M_].z)) / bSpline2LUT_[3 *
							// nSamplesPerSegment_];
							double ckMz = coef_[M_ * (M_ - 1) + 1].z;
		...
		for (int i = 0; i < MR_; i++) {
			for (int j = 0; j <= MR_; j++) {
				...
				if (index_j >= 0 && index_j < NR2_) {
					...
					for (int k = 0; k < M_; k++) {
						...
						if (index_i >= 0 && index_i < NR2_) {
							...
							basisFactor = aux * bSpline1DerivativeLUT_[index_i];
							xPosVal += ckminus1x * basisFactor;
							yPosVal += ckminus1y * basisFactor;
							zPosVal += ckminus1z * basisFactor;
						}
				...
				if (index_j >= 0 && index_j < NR2_) {
					...
					for (int k = 0; k < M_; k++) {
						...
						if (index_i >= 0 && index_i < NR2_) {
							...
							/*
							 * double ck0x = (coef_[M_ * (M_ - 1)].x -
							 * bSpline2LUT_[5 * nSamplesPerSegment_] (ckminus1x
							 * + coef_[k].x)) / bSpline2LUT_[3 *
							 * nSamplesPerSegment_];
							 */
							double ck0x = coef_[M_ * (M_ - 1)].x;
							...
							/*
							 * double ck0y = (coef_[M_ * (M_ - 1)].y -
							 * bSpline2LUT_[5 * nSamplesPerSegment_] (ckminus1y
							 * + coef_[k].y)) / bSpline2LUT_[3 *
							 * nSamplesPerSegment_];
							 */
							double ck0y = coef_[M_ * (M_ - 1)].y;
							...
							/*
							 * double ck0z = (coef_[M_ * (M_ - 1)].z -
							 * bSpline2LUT_[5 * nSamplesPerSegment_] (ckminus1z
							 * + coef_[k].z)) / bSpline2LUT_[3 *
							 * nSamplesPerSegment_];
							 */
							double ck0z = coef_[M_ * (M_ - 1)].z;
				...
				if (index_j >= 0 && index_j < NR2_) {
					aux = bSpline2LUT_[index_j];
					for (int k = 0; k < M_; k++) {
						index_i = (2 * i + nSamplesPerSegment_ * (3 - 2 * k) + MR2_)
								% MR2_;
						if (index_i >= 0 && index_i < NR2_) {
							// compute c[k,M+1]
							/*
		...
		for (int i = 0; i < MR_; i++) {
			for (int j = 0; j <= MR_; j++) {
				...
				for (int l = 1; l <= M_ - 1; l++) {
					...
					index_v = 2 * j + nSamplesPerSegment_ * (4 - 2 * l);

					if (index_v >= 0 && index_v < NR2_) {
						aux = bSpline2LUT_[index_v];
						for (int k = 0; k < M_; k++) {
							// index_u = (2 * i + nSamplesPerSegment_ * (3 - 2 *
							// k) + MR2_) % MR2_;
				...
				index_v = 2 * j + nSamplesPerSegment_ * 6;

				if (index_v >= 0 && index_v < NR2_) {
					aux = bSpline2LUT_[index_v];
					for (int k = 0; k < M_; k++) {
						// index_u = (2 * i + nSamplesPerSegment_ * (3 - 2 * k)
						// + MR2_) % MR2_;
						index_u = (2 * i + nSamplesPerSegment_ * (4 - 2 * k) + MR2_)
								% MR2_;
				...
				index_v = 2 * j + nSamplesPerSegment_ * 4;
				...
				index_v = 2 * j + nSamplesPerSegment_ * (2 - 2 * M_);
				...
				index_v = 2 * j + nSamplesPerSegment_ * (4 - 2 * M_);
	...
	// TODO: check if this function also needs to be updated w.r.t. the MSpline
